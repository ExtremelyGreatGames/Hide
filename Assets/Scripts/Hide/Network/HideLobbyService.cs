...
namespace Hide.Network
{
    ...
    public class HideLobbyService : INetworkBehavior
    {
        ...
        private static HideLobbyService _instance;

        /// <summary>
        /// The instance of the Lobby Service
        /// </summary>
        public static HideLobbyService Instance
        {
            get
            {
                if (_instance == null)
                    _instance = new HideLobbyService();
                return _instance;
            }
        }
        ...
        private IHideLobbyMaster _masterLobby;

        /// <summary>
        /// The master lobby in which we will be doing callback to
        /// </summary>
        public IHideLobbyMaster MasterLobby
        {
            get
            {
        ...
        private IClientHidePlayer _myself;

        /// <summary>
        /// Reference to my networking player
        /// </summary>
        public IClientHidePlayer MyMockPlayer
        {
            get
            {
                if (_myself == null)
        ...
        /// </summary>
        private class DummyPlayer : IClientHidePlayer
        {
        ...
        /// </summary>
        private class DummyLobbyMaster : IHideLobbyMaster
        {
            ...
            private Dictionary<int, List<IClientHidePlayer>> _lobbyTeams =
                new Dictionary<int, List<IClientHidePlayer>>();
            ...
            public DummyLobbyMaster()
            {
                ...
                LobbyTeams.Add(0, new List<IClientHidePlayer>() {player});
            }
            ...
            public void OnFNLobbyMasterKnowledgeTransfer(IHideLobbyMaster previousLobbyMaster)
            {
                ...
                {
                    IClientHidePlayer player = previousLobbyMaster.LobbyPlayers[i];
                    LobbyPlayers.Add(player);
                    LobbyPlayersMap.Add(player.NetworkId, player);
                }
                ...
                foreach (KeyValuePair<int, List<IClientHidePlayer>> kv in previousLobbyMaster.LobbyTeams)
                {
                    LobbyTeams.Add(kv.Key, kv.Value);
                }
            }
        ...
        public void SetLobbyMaster(IHideLobbyMaster lobbyMaster)
        {
        ...
        {
            IClientHidePlayer player = GetClientMockPlayer(networkObject.MyPlayerId);
            player.Name = newName;
        ...
        //public void PlayerConnected(IClientHidePlayer player)
        //{
        ...
        private IClientHidePlayer CreateClientMockPlayer(uint playerId, string playerName)
        {
            var player = new DummyPlayer(playerId, playerName, 0, 0);
            return player;
        }

        private IClientHidePlayer GetClientMockPlayer(uint playerId)
        {
            IClientHidePlayer targetPlayer = null;
            if (MasterLobby != null)
            {
                for (int i = 0; i < MasterLobby.LobbyPlayers.Count; ++i)
                {
        ...
        private void PlayerConnected(NetworkingPlayer player, NetWorker sender)
        {
            ...
            sender.IteratePlayers((p) =>
            {
                ...
                IClientHidePlayer cPlayer = MasterLobby.LobbyPlayers.First(l => l.NetworkId == p.NetworkId);
