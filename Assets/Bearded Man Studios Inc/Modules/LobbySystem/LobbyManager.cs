...
namespace BeardedManStudios.Forge.Networking.Unity.Lobby
{
    public class LobbyManager : MonoBehaviour, ILobbyMaster
    {
        ...
        private void CheckForService(NetWorker networker, int identity, uint id, Frame.FrameStream frame,
        ...
        public void StartGame(int sceneID)
        {
            if (NetworkManager.Instance.IsServer)
            {
                foreach (var lobbyPlayer in LobbyService.Instance.MasterLobby.LobbyPlayers)
                {
                    Debug.Log(lobbyPlayer.NetworkId);
                }   
        ...
        public void OnFNLobbyMasterKnowledgeTransfer(ILobbyMaster previousLobbyMaster)
        {
            ...
            while (iterTeams.MoveNext())
            {
                ...
                {
                    KeyValuePair<int, List<IClientMockPlayer>> kv =
                        (KeyValuePair<int, List<IClientMockPlayer>>) iterTeams.Current;
            ...
            while (iterPlayersMap.MoveNext())
            {
                ...
                {
                    KeyValuePair<uint, IClientMockPlayer> kv =
                        (KeyValuePair<uint, IClientMockPlayer>) iterPlayersMap.Current;
